...
async function main() {
    ...
    for (let i = 0; i < numBeans; i++) {
        let beanTranslate = [rand(-50, 50), rand(-50, 50), rand(-20, -10)];
        beans.push(beanTranslate);
    }
    ...
    let render = () => {
        ...
        viewMatrix = getViewMatrix(radius, angle.x / 4, angle.y / 16, boardDim);
        projectionMatrix = getProjectionMatrix(zoom, near, far, boardDim);

        renderScene(sceneProgram, viewMatrix, projectionMatrix, boardObj);

        (beanViewMatrix = getViewMatrix(1, angle.x / 4, angle.y / 16, beanDim)),
            (beanProjectionMatrix = getProjectionMatrix(
                zoom,
                near,
                far,
                beanDim
        ...
        beans.forEach(function (beanTranslate) {
            renderBeans(
                beanProgram,
                beanViewMatrix,
...
function getViewMatrix(r, x_angle, y_angle, model) {
    const gazeDirection = m4.transformDirection(
        m4.multiply(m4.rotationY(x_angle), m4.rotationX(-y_angle)),
        [0, 0, 1]
    );
